class Applic < ActiveRecord::Base
  ...
  def fio_eng_cannot_be_blank
    if !user.is_antok_member? && fio_eng.blank?
      errors.add(:fio_eng, :blank)
    end
  end

  def birth_date_cannot_be_blank
    if !user.is_antok_member? && birth_date.blank?
